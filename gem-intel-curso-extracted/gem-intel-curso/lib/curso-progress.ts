...
export async function usuarioTemAcesso(userId: string, cursoSlug = CURSO_SLUG): Promise<boolean> {
...
export async function getProgressoCurso(userId: string, cursoSlug = CURSO_SLUG) {
  const { data } = await supabase
    .from('progresso_aulas')
